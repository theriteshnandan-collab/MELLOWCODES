...
export default function ProcessGrid() {
  ...
  return (
    <section id="process" className="py-60 bg-[#111111] relative overflow-hidden">
      <div className="max-w-[1600px] mx-auto px-6 lg:px-32">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-12 mb-40">
          <div className="max-w-[800px]">
            ...
            <h2 className="text-8xl md:text-[10rem] font-black text-white leading-[0.8] tracking-tighter" style={{ fontFamily: "var(--font-space-grotesk)" }}>
              Methodical<br />
              <span className="italic text-white/5" style={{ WebkitTextStroke: "1px #FFFFFF" }}>Conquest.</span>
            </h2>
          </div>
          <p className="text-white/40 text-2xl font-light leading-relaxed max-w-sm mb-4">
          ...
          {steps.map((step, i) => (
            ...
            >
              <motion.div
                ...
                className="relative aspect-[3/4] rounded-3xl bg-[#181818] border border-white/5 overflow-hidden transition-all duration-700 shadow-sm hover:shadow-xl cursor-pointer"
                ...
                <div className="absolute inset-0 p-8 md:p-12 flex flex-col justify-between overflow-hidden">
                  <div className="flex justify-between items-start relative z-10 px-2">
                    <div className="flex flex-col gap-2 transition-transform duration-700">
                      <motion.h3 layoutId={`title-${step.id}`} transition={{ duration: 0.8, ease: [0.19, 1, 0.22, 1] }} className="text-3xl md:text-4xl font-black text-white uppercase leading-tight" style={{ fontFamily: "var(--font-space-grotesk)" }}>{step.title}</motion.h3>
                    </div>
                  ...
                  <div className="relative w-full aspect-square flex items-center justify-center py-4 flex-shrink min-h-[100px]">
                    ...
                    <motion.div layoutId={`image-${step.id}`} transition={{ duration: 0.8, ease: [0.19, 1, 0.22, 1] }} className="relative w-[75%] h-[75%] transition-all duration-1000 ease-[0.19,1,0.22,1] z-20">
                      <Image
                        src={step.img}
                        alt={step.title}
                        fill
                  ...
                  <div className="relative z-10 px-2 mt-auto transition-transform duration-700">
                    <motion.p layoutId={`desc-${step.id}`} transition={{ duration: 0.8, ease: [0.19, 1, 0.22, 1] }} className="text-white/50 transition-colors duration-700 text-sm md:text-base leading-relaxed font-medium">
                      {step.desc}
                    </motion.p>
                  </div>
                </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 relative flex items-center justify-center p-12 bg-white/50">
                    ...
                    <motion.div layoutId={`image-${step.id}`} transition={{ duration: 0.8, ease: [0.19, 1, 0.22, 1] }} className="relative w-[90%] h-[90%] z-20">
                      <Image
                        src={step.img}
                        alt={step.title}
                        fill
